...
setwd("/home/sylvester/test/PG0102b1w-STDncodseq02B-01")
...
b1STDnCoda02 <- new.env(parent = emptyenv())
load("/home/pvannameihome/PG0102b1a-STDncodseq02B-01/dada2f230/202105-pg0102b1a-16S-rmchloMTunc-Q1f230-STDnCoda01-minprop-001minOc01OTUs1615_allphyloseq.Rdata",
     envir = b1STDnCoda02)


#original tax_tab
tax_tab = b1STDnCoda02$tax_tab
tax_tab.chlo = b1STDnCoda02$tax_tab.chlo

##asvid
tax_tab.chlo.asvid = b1STDnCoda02$tax_tab.chlo.asvid
tax_tab.MT.asvid = b1STDnCoda02$tax_tab.MT.asvid
tax_tab.rick.unc.asvid  = b1STDnCoda02$tax_tab.rick.unc.asvid 
tax_tab.MTunc.asvid = b1STDnCoda02$tax_tab.MTunc.asvid


phyq1PG0102b1a = b1STDnCoda02$phyq1PG0102b1a
asvsums = taxa_sums(phyq1PG0102b1a)
length(which(asvsums > 0))
length(which(asvsums == 0))

kepasv.a = names(asvsums[which(asvsums > 0)])
length(kepasv.a)

## all PG0102b1a excluding 0 ASVs
phyq1PG0102b1a0 = prune_taxa(kepasv.a, phyq1PG0102b1a)
phyq1PG0102b1a0 
## write down numbers of taxa and samples, variables

## source for wa
phyq1PG0102b1w = subset_samples(phyq1PG0102b1a0, source == "wa")

asvsums = taxa_sums(phyq1PG0102b1w)
length(which(asvsums > 0))
length(which(asvsums == 0))

kepasv.w = names(asvsums[which(asvsums > 0)])
length(kepasv.w)

phyq1PG0102b1w = prune_taxa(kepasv.w, phyq1PG0102b1w)
phyq1PG0102b1w
## write down numbers of taxa and samples, variables
## otu_table()   OTU Table:         [ 10977 taxa and 64 samples ]
## sample_data() Sample Data:       [ 64 samples by 11 sample variables ]

#update tax_tab
tax_tab = tax_tab[kepasv.w,]
count_tab <- data.frame(otu_table(phyq1PG0102b1w))

rm(list = ls(b1STDnCoda02, all.names = TRUE), envir =  b1STDnCoda02)
ls(b1STDnCoda02, all.names = TRUE)

sample_info_tab = sample_data(phyq1PG0102b1w)


### output directory within the working directory
outdir = "/home/sylvester/test/PG0102b1w-STDncodseq02B-01/dada2f230/"
...
fcount_tab <- codaSeq.filter(count_tab, min.reads=5000, min.prop=0.01, min.occurrence=0.03, samples.by.row=FALSE)
...
fcount_tab <- codaSeq.filter(count_tab, min.reads=5000, 
                             min.prop=0.001, min.occurrence=0.03, samples.by.row=FALSE)
## for ALL using this.   
fminprop = "001"

##nbrtotal OTU
nbrOTUs = dim(fcount_tab)[1]	#1027
#######################################

###########  
## file prefix
fileprefx = paste("202105-", fpond, "-", 
                  ftiss, "-", 
...
#### p.0001
fcount_tab <- codaSeq.filter(count_tab, min.reads=5000, 
                             min.prop=0.0001, min.occurrence=0.03, samples.by.row=FALSE)
## for ALL using this.
fminprop = "0001"

##nbrtotal OTU
nbrOTUs = dim(fcount_tab)[1]	#2966
...
p.cmb.ch.saminfo$idno = paste(p.cmb.ch.saminfo$source,
                         p.cmb.ch.saminfo$shrimpid, 
                         p.cmb.ch.saminfo$site, p.cmb.ch.saminfo$filter,  
                         sep="-")

p.cmb.ch.saminfo$pgpd = paste(p.cmb.ch.saminfo$pg,
                         p.cmb.ch.saminfo$pd,
                         sep="-")
...
ggsave(filename=paste("202105-", fpond, "-", 
                      ftiss, "-", 
...
p.cmb.MTunc.saminfo$idno = paste( p.cmb.MTunc.saminfo$source,
                              p.cmb.MTunc.saminfo$shrimpid, 
                              p.cmb.MTunc.saminfo$site, p.cmb.MTunc.saminfo$filter,  
                              sep="-")
															
p.cmb.MTunc.saminfo$pgpd = paste(p.cmb.MTunc.saminfo$pg,
                              p.cmb.MTunc.saminfo$pd,
                              sep="-")
...
##   writeout info
write.table(p.cmb.ch.saminfo, file=paste("202105-", fpond, "-", 
                                          ftiss, "-", 
                                          prop1, "_pctchloro.tab", sep = ""), 
            															sep = "\t", quote = FALSE)

write.table(p.cmb.MTunc.saminfo, file=paste("202105-", fpond, "-", 
                                         ftiss, "-", 
                                         prop1, "_pctMTunc.tab", sep = ""), 
            														 sep = "\t", quote = FALSE)

write.table(cmb.ch, file=paste("202105-", fpond, "-", 
                                         ftiss, "-", 
...
write.table(cmb.ASVch, file=paste("202105-", fpond, "-", 
                               ftiss, "-", 
...
f2.PG0102b1wphyseq <- phyloseq(OTU, TAX, samples)
### phyloseq with the counts.

f2relCZM.PG0102b1wphyseq = f2.PG0102b1wphyseq

otu_mat <- as.matrix(t(f.n0.count_tab))  ### transposed f.n0

otu_table(f2relCZM.PG0102b1wphyseq) = otu_table(otu_mat, taxa_are_rows = TRUE)

f2relCZM.PG0102b1wphyseq ## phyloseq with estimated 0 count from CZM method

### codaSeq.clr results
f2relCodaCRL.PG0102b1wphyseq = f2.PG0102b1wphyseq

otu_mat <- as.matrix(t(f.n0.clr))  ### transposed f.n0.clr

otu_table(f2relCodaCRL.PG0102b1wphyseq) = otu_table(otu_mat, taxa_are_rows = TRUE)

## phyloseq with CodaCRL


### microbiome transform cleer
f2rel.microCLR.PG0102b1wphyseq = microbiome::transform(f2.PG0102b1wphyseq, 'clr')

#### remove chlo ONLY
f2count_tab.chlo.rowid = which(row.names(otu_table(f2.PG0102b1wphyseq)) %in% 
                                 c(tax_tab.chlo.asvid ))

chlor_otu_tab = subset(otu_table(f2.PG0102b1wphyseq), 
                       rownames(otu_table(f2.PG0102b1wphyseq)) %in% 
                       c(tax_tab.chlo.asvid ))
												 
f2.PG0102b1w_chlorONLY_physeq = merge_phyloseq(chlor_otu_tab, 
                                          tax_table(f2.PG0102b1wphyseq), 
                                          sample_data(f2.PG0102b1wphyseq))
																					
#### get CMZ 

chlor_otu_tab = subset(otu_table(f2relCZM.PG0102b1wphyseq), 
                       rownames(otu_table(f2relCZM.PG0102b1wphyseq)) %in% 
                       c(tax_tab.chlo.asvid))
												 
f2relCZM.PG0102b1w_chlorONLY_physeq = merge_phyloseq(chlor_otu_tab, 
                                                	tax_table(f2relCZM.PG0102b1wphyseq), 
                                               	 	sample_data(f2relCZM.PG0102b1wphyseq))


## print tax 
plot_bar(f2relCZM.PG0102b1w_chlorONLY_physeq, fill="Family") + theme_bw() +  theme(axis.text.x = element_text(angle = 90))
	
ggsave(filename=paste(fileprefx, "-chloONLY", "_familyAbundance.pdf", sep = ""), width=420, height=230, units="mm") 

#### remove MTunc ONLY
f2count_tab.MTunc.rowid = which(row.names(otu_table(f2.PG0102b1wphyseq)) %in% c( tax_tab.MTunc.asvid ))

MTunc_otu_tab = subset(otu_table(f2.PG0102b1wphyseq), 
                       rownames(otu_table(f2.PG0102b1wphyseq)) %in% 
                       c(tax_tab.MTunc.asvid ))
												 
f2.PG0102b1w_MTunc_physeq = merge_phyloseq(MTunc_otu_tab, 
                                          tax_table(f2.PG0102b1wphyseq), 
                                          sample_data(f2.PG0102b1wphyseq))
...
MTunc_otu_tab = subset(otu_table(f2relCZM.PG0102b1wphyseq), 
                       rownames(otu_table(f2relCZM.PG0102b1wphyseq)) %in% 
                       c(tax_tab.MTunc.asvid ))
												 
f2relCZM.PG0102b1w_MTuncONLY_physeq = merge_phyloseq(MTunc_otu_tab, 
                                                tax_table(f2relCZM.PG0102b1wphyseq), 
                                                sample_data(f2relCZM.PG0102b1wphyseq))
																								

## print tax 
plot_bar(f2relCZM.PG0102b1w_MTuncONLY_physeq, fill="Genus") +theme_bw() +  theme(axis.text.x = element_text(angle = 90))

ggsave(filename=paste(fileprefx, "-MTuncONLY", "_GenusAbundance.pdf", sep = ""), width=420, height=230, units="mm") 

### remove chlo & MTunc
f2count_tab.chlo.rowid = which(row.names(otu_table(f2.PG0102b1wphyseq)) %in% c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid ))

chlor_otu_tab = subset(otu_table(f2.PG0102b1wphyseq), 
                       rownames(otu_table(f2.PG0102b1wphyseq)) %in% 
                       c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid ))
												 
f2.PG0102b1w_chlor_physeq = merge_phyloseq(chlor_otu_tab, 
                                              tax_table(f2.PG0102b1wphyseq), 
                                              sample_data(f2.PG0102b1wphyseq))

filt_otu_tab = subset(otu_table(f2.PG0102b1wphyseq), 
                      !(rownames(otu_table(f2.PG0102b1wphyseq)) %in% 
                          c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid )))

f2.PG0102b1w_NonChl_physeq = merge_phyloseq(filt_otu_tab, 
                                               tax_table(f2.PG0102b1wphyseq), 
                                               sample_data(f2.PG0102b1wphyseq))


#### get CMZ f2relCZM.PG0102b1wphyseq

chlor_otu_tab = subset(otu_table(f2relCZM.PG0102b1wphyseq), 
                       rownames(otu_table(f2relCZM.PG0102b1wphyseq)) %in% 
                       c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid ))
f2relCZM.PG0102b1w_chlor_physeq = merge_phyloseq(chlor_otu_tab, 
                                                    tax_table(f2relCZM.PG0102b1wphyseq), 
                                                    sample_data(f2relCZM.PG0102b1wphyseq))

filt_otu_tab = subset(otu_table(f2relCZM.PG0102b1wphyseq), 
                      !(rownames(otu_table(f2relCZM.PG0102b1wphyseq)) %in% 
                          c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid )))
													
f2relCZM.PG0102b1w_NonChl_physeq = merge_phyloseq(filt_otu_tab, 
                                                     tax_table(f2relCZM.PG0102b1wphyseq), 
                                                     sample_data(f2relCZM.PG0102b1wphyseq))
...
save(
	  fcount_tab,
	  f2.PG0102b1wphyseq, 
	  f2rel.microCLR.PG0102b1wphyseq,
	  f2relCodaCRL.PG0102b1wphyseq,
	  f2relCZM.PG0102b1wphyseq,
	  filt_count_tab,
	  ...
	  f2.PG0102b1w_chlorONLY_physeq,
	  f2.PG0102b1w_MTunc_physeq,
	  f2.PG0102b1w_NonChl_physeq,
	  f2relCZM.PG0102b1w_chlorONLY_physeq,
	  f2relCZM.PG0102b1w_MTuncONLY_physeq,
	  f2relCZM.PG0102b1w_NonChl_physeq,
	  tax_tab.chlo.asvid, 
	  tax_tab.MTunc.asvid,
	  tax_tab.MT.asvid,

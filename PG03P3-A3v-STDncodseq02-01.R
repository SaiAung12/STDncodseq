...
setwd("/home/sylvester/test/PG03P3-A3v-STDncodseq02-01")
...
input_qza = "/mnt/zoeahome/PG03P3-cmb/PG03P3-cmb-02c/dada2f230_pg03p3ds012316s_rep_silvaV3V4-97_tax.qza"
input_tab = "/mnt/zoeahome/PG03P3-A3/tax/dada2f230_pg03p3ds012316s_rep_silvaV3V4-97_tax.tab"
input_chlo = "/mnt/zoeahome/PG03P3-A3/tax/dada2f230_pg03p3ds012316s_rep_silvaV3V4-97_tax.Chlo"
input_mit = "/mnt/zoeahome/PG03P3-A3/tax/dada2f230_pg03p3ds012316s_rep_silvaV3V4-97_tax.mitch"
input_cul = "/mnt/zoeahome/PG03P3-A3/tax/dada2f230_pg03p3ds012316s_rep_silvaV3V4-97_tax.ricket.uncul"
input_tsv = "/mnt/zoeahome/PG03P3-A3/tsv/20210623-PG03P3-a3-metadata.tsv"

load("/home/sylvester/test/PG03P3-A3-N3/202106-pg03P3-A3-N3.Rdata")

# This command should work as-is with Greengenes.
#phyq1PG03 <- qza_to_phyloseq(features=input_qza, metadata=input_tsv )

count_tab <- data.frame(otu_table(phyq1PG03A3v))
...
sample_info_tab = sample_data(phyq1PG03A3v)


### output directory within the working directory
outdir = "/home/sylvester/test/PG03P3-A3v-STDncodseq02-01/dada2f230/"
...
### modify pd value of week0 samples
## copy old pd to new pdfx variables
filt_sample_info_tab$pdfx = filt_sample_info_tab$pd

## change the pd of week0
filt_sample_info_tab[which(filt_sample_info_tab$pd == "p3a" &
                     filt_sample_info_tab$wk == "0"), "pdfx" ] = "p3b"
filt_sample_info_tab[which(filt_sample_info_tab$pd == "p3b" &
                     filt_sample_info_tab$wk == "0"), "pdfx" ] = "p3a"
...
fcount_tab <- codaSeq.filter(count_tab, min.reads=5000, min.prop=0.01, min.occurrence=minoc, samples.by.row=FALSE)
...
fcount_tab <- codaSeq.filter(count_tab, min.reads=5000, 
                             min.prop=0.001, min.occurrence=minoc, samples.by.row=FALSE)
## for ALL using this.   
fminprop = "001"

##nbrtotal OTU
nbrOTUs = dim(fcount_tab)[1]	#1417
#######################################

###########  
## file prefix
fileprefx = paste(date, fpond, "-", 
                  ftiss, "-", 
...
#### p.0001
fcount_tab <- codaSeq.filter(count_tab, min.reads=5000, 
                             min.prop=0.0001, min.occurrence=minoc, samples.by.row=FALSE)
## for ALL using this.
fminprop = "0001"

##nbrtotal OTU
nbrOTUs = dim(fcount_tab)[1]	#5708
...
ggsave(filename=paste(date, fpond, "-", 
                      ftiss, "-", 
                      prop1, "_pctchloro_label_pdfx.pdf", sep = ""), 
...
ggsave(filename=paste(date, fpond, "-", 
                      ftiss, "-", 
                      prop1, "_pctchloro_xbel_pdfx.pdf", sep = ""), 
      								width=390, height=210, units="mm")
...
ggsave(filename=paste(date, fpond, "-", 
                      ftiss, "-", 
                      prop1, "_pctMTunc_label_pdfx.pdf", sep = ""), 
       							  width=390, height=210, units="mm")
...
ggsave(filename=paste(date, fpond, "-", 
                      ftiss, "-", 
                      prop1, "_pctMTunc_xbel_pdfx.pdf", sep = ""), 
       							  width=390, height=210, units="mm")

##   writeout info
write.table(p.cmb.ch.saminfo, file=paste(date, fpond, "-", 
                                          ftiss, "-", 
                                          prop1, "_pctchloro.tab", sep = ""), 
            															sep = "\t", quote = FALSE)

write.table(p.cmb.MTunc.saminfo, file=paste(date, fpond, "-", 
                                         ftiss, "-", 
                                         prop1, "_pctMTunc.tab", sep = ""), 
            														 sep = "\t", quote = FALSE)

write.table(cmb.ch, file=paste(date, fpond, "-", 
                                         ftiss, "-", 
...
write.table(cmb.ASVch, file=paste(date, fpond, "-", 
                               ftiss, "-", 
...
### please also change the prefix file e.g., PG03p301 to pg0304fspc1Q1 for all the corresponding objects / variables

f2.PG03p3Q1physeq <- phyloseq(OTU, TAX, samples)
### phyloseq with the counts.

f2relCZM.PG03p3Q1physeq = f2.PG03p3Q1physeq

otu_mat <- as.matrix(t(f.n0.count_tab))  ### transposed f.n0

otu_table(f2relCZM.PG03p3Q1physeq) = otu_table(otu_mat, taxa_are_rows = TRUE)

f2relCZM.PG03p3Q1physeq ## phyloseq with estimated 0 count from CZM method

### codaSeq.clr results
f2relCodaCRL.PG03p3Q1physeq = f2.PG03p3Q1physeq

otu_mat <- as.matrix(t(f.n0.clr))  ### transposed f.n0.clr

otu_table(f2relCodaCRL.PG03p3Q1physeq) = otu_table(otu_mat, taxa_are_rows = TRUE)

## phyloseq with CodaCRL


### microbiome transform cleer
f2rel.microCLR.PG03p3Q1physeq = microbiome::transform(f2.PG03p3Q1physeq, 'clr')

#### remove chlo ONLY
f2count_tab.chlo.rowid = which(row.names(otu_table(f2.PG03p3Q1physeq)) %in% 
                                 c(tax_tab.chlo.asvid ))

chlor_otu_tab = subset(otu_table(f2.PG03p3Q1physeq), 
                       rownames(otu_table(f2.PG03p3Q1physeq)) %in% 
                       c(tax_tab.chlo.asvid ))
												 
f2.PG03p3Q1_chlorONLY_physeq = merge_phyloseq(chlor_otu_tab, 
                                          tax_table(f2.PG03p3Q1physeq), 
                                          sample_data(f2.PG03p3Q1physeq))
																					
#### get CMZ 

chlor_otu_tab = subset(otu_table(f2relCZM.PG03p3Q1physeq), 
                       rownames(otu_table(f2relCZM.PG03p3Q1physeq)) %in% 
                       c(tax_tab.chlo.asvid))
												 
f2relCZM.PG03p3Q1_chlorONLY_physeq = merge_phyloseq(chlor_otu_tab, 
                                                	tax_table(f2relCZM.PG03p3Q1physeq), 
                                               	 	sample_data(f2relCZM.PG03p3Q1physeq))


## print tax 
plot_bar(f2relCZM.PG03p3Q1_chlorONLY_physeq, fill="Family") + theme_bw() +  theme(axis.text.x = element_text(angle = 90))
	
ggsave(filename=paste(fileprefx, "-chloONLY", "_familyAbundance.pdf", sep = ""), width=420, height=230, units="mm") 

#### remove MTunc ONLY
f2count_tab.MTunc.rowid = which(row.names(otu_table(f2.PG03p3Q1physeq)) %in% c( tax_tab.MTunc.asvid ))

MTunc_otu_tab = subset(otu_table(f2.PG03p3Q1physeq), 
                       rownames(otu_table(f2.PG03p3Q1physeq)) %in% 
                       c(tax_tab.MTunc.asvid ))
												 
f2.PG03p3Q1_MTunc_physeq = merge_phyloseq(MTunc_otu_tab, 
                                          tax_table(f2.PG03p3Q1physeq), 
                                          sample_data(f2.PG03p3Q1physeq))
...
MTunc_otu_tab = subset(otu_table(f2relCZM.PG03p3Q1physeq), 
                       rownames(otu_table(f2relCZM.PG03p3Q1physeq)) %in% 
                       c(tax_tab.MTunc.asvid ))
												 
f2relCZM.PG03p3Q1_MTuncONLY_physeq = merge_phyloseq(MTunc_otu_tab, 
                                                tax_table(f2relCZM.PG03p3Q1physeq), 
                                                sample_data(f2relCZM.PG03p3Q1physeq))
																								

## print tax 
plot_bar(f2relCZM.PG03p3Q1_MTuncONLY_physeq, fill="Genus") +theme_bw() +  theme(axis.text.x = element_text(angle = 90))

ggsave(filename=paste(fileprefx, "-MTuncONLY", "_GenusAbundance.pdf", sep = ""), width=420, height=230, units="mm") 

### remove chlo & MTunc
f2count_tab.chlo.rowid = which(row.names(otu_table(f2.PG03p3Q1physeq)) %in% c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid ))

chlor_otu_tab = subset(otu_table(f2.PG03p3Q1physeq), 
                       rownames(otu_table(f2.PG03p3Q1physeq)) %in% 
                       c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid ))
												 
f2.PG03p3Q1_chlor_physeq = merge_phyloseq(chlor_otu_tab, 
                                              tax_table(f2.PG03p3Q1physeq), 
                                              sample_data(f2.PG03p3Q1physeq))

filt_otu_tab = subset(otu_table(f2.PG03p3Q1physeq), 
                      !(rownames(otu_table(f2.PG03p3Q1physeq)) %in% 
                          c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid )))

f2.PG03p3Q1_NonChl_physeq = merge_phyloseq(filt_otu_tab, 
                                               tax_table(f2.PG03p3Q1physeq), 
                                               sample_data(f2.PG03p3Q1physeq))


#### get CMZ f2relCZM.PG03p3Q1physeq

chlor_otu_tab = subset(otu_table(f2relCZM.PG03p3Q1physeq), 
                       rownames(otu_table(f2relCZM.PG03p3Q1physeq)) %in% 
                       c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid ))
f2relCZM.PG03p3Q1_chlor_physeq = merge_phyloseq(chlor_otu_tab, 
                                                    tax_table(f2relCZM.PG03p3Q1physeq), 
                                                    sample_data(f2relCZM.PG03p3Q1physeq))

filt_otu_tab = subset(otu_table(f2relCZM.PG03p3Q1physeq), 
                      !(rownames(otu_table(f2relCZM.PG03p3Q1physeq)) %in% 
                          c(tax_tab.chlo.asvid, tax_tab.MTunc.asvid )))
													
f2relCZM.PG03p3Q1_NonChl_physeq = merge_phyloseq(filt_otu_tab, 
                                                     tax_table(f2relCZM.PG03p3Q1physeq), 
                                                     sample_data(f2relCZM.PG03p3Q1physeq))
...
save(
  fcount_tab,
  f2.PG03p3Q1physeq, 
  f2rel.microCLR.PG03p3Q1physeq,
  f2relCodaCRL.PG03p3Q1physeq,
  f2relCZM.PG03p3Q1physeq,
  filt_count_tab,
  ...
  f2.PG03p3Q1_chlorONLY_physeq,
  f2.PG03p3Q1_MTunc_physeq,
  f2.PG03p3Q1_NonChl_physeq,
  f2relCZM.PG03p3Q1_chlorONLY_physeq,
  f2relCZM.PG03p3Q1_MTuncONLY_physeq,
  f2relCZM.PG03p3Q1_NonChl_physeq,
  tax_tab.chlo.asvid, 
  tax_tab.MTunc.asvid,
  tax_tab.MT.asvid,
